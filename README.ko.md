...
5. **변경은 외과적으로.** 작업이 요구하는 부분만 수정하라. 기존 스타일을 따르라. 무관한 코드를 리팩터링/이름 변경/포맷/정리하지 마라.
...
- 결정의 근거를 `docs/changelog/changelog-YYYY-MM-DD.md`에 기록하라.
- 독립적인 작업은 새 컨텍스트의 서브에이전트에 위임하라. 병렬 읽기는 한 턴에 묶어라.
