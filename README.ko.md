...
3. **선택지를 제시하라.** 구현 전에 합리적인 접근 두세 가지를 제시하고 가장 단순하고 지속 가능한 것을 추천하라.
4. **탐색한 다음 작은 단계로 계획하라.** 변경을 제안하기 전에 관련 코드를 먼저 읽어라. 검증 가능한 단계로 쪼개고, 각 단계에는 자체 점검을 포함하라.
5. **변경은 외과적으로.** 작업이 요구하는 부분만 만져라. 기존 스타일을 따르라. 무관한 코드를 리팩터링/이름 변경/포맷/정리하지 마라.
6. **재발명 전에 재사용; 단순함을 택하라.** 기존 유틸·패턴·파일을 먼저 검색하라. 문제를 올바르게 푸는 최소 코드를 써라. 추측성 기능, 일반화 추상화, 불필요한 설정 가능성을 피하라.
...
10계명이 *에이전트가 무엇을 하는가* 를 다룬다면, 이 섹션은 *어떻게 말하는가* 를 다룬다 — 응답, 커밋 메시지, 문서 갱신, 코드 주석에 모두 적용.

- 결정/답부터 먼저. 그다음 *이유(why)*를 한 절로 짧게.
- 군더더기 절단: 문장보다 핵심 키워드. 맥락에서 자명한 건 잘라라.
- *무엇(what)* 은 코드가 말한다. *왜(why)* 는 응답·커밋 메시지·주석이 말한다.
- 주석: 코드만으로 *이유*가 드러나지 않을 때만 써라. 보통 한 줄이면 충분.
- 주니어 엔지니어도 이해할 수 있는 단어를 써라. 전문 용어는 처음 등장할 때 한 줄로 풀어줘라.
...
- 관계없는 작업 사이엔 컨텍스트를 비워라. 누적된 실패 시도는 다음 시도를 오염시킨다.
- 결정의 근거를 `log/changelog-YYYY-MM-DD.md`에 기록.
