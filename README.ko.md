...
- 결정과 이유는 말하지 않고 서술만 길게 푸는 에이전트
- 메인 컨텍스트에 코드베이스 전체를 끌어와 같은 설명을 또 요구하는 에이전트

10계명 — 그리고 짧은 *Response & Documentation Style* 섹션 — 이 모든 실패를 막는 최소 규칙 모음이다.
...
2. **불확실성을 드러내고 선택지를 제시하라.** 요건이 모호하면 물어라. 해석이 여러 개면 합리적인 접근 두세 가지와 함께 제시하고, 가장 단순하고 지속 가능한 것을 추천하라. 위험하면 위험하다고 말하라.
3. **단위는 작고 응집력 있게.** 파일 하나 = 목적 하나, 함수 하나 = 일 하나. 파일 ~200–400줄, 함수 50줄 미만, 중첩 4단계 이하를 지향하라. 임계값 초과는 위반이 아니라 *리팩터 신호* — 응집(cohesion)이 줄 수보다 우선이다. 타입이 아니라 기능/도메인으로 쪼개라.
...
- 독립 작업은 새 컨텍스트의 서브에이전트에 위임하라. 병렬 읽기는 한 턴에 묶어라.
...
- **부정 예시는 비용이 클 때만.** 3, 5, 6, 7, 9, 10번은 실패 모드를 명시 — 모호한 긍정문("주의하라")은 긴 컨텍스트에서 살아남지 못한다.
- **도구 이름 없음.** `pytest`, `npm`, `cargo`, `gh` 같은 이름이 안 들어간다. 어느 스택에도 휴대 가능.
- **행동 vs 의사소통.** 10계명은 *에이전트가 무엇을 하는가*. Response & Documentation Style은 *어떻게 말하는가*. 청중이 다르면 룰도 분리.
